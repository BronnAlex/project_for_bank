...
def filter_by_currency(transaction_list: list[dict[str, object]], current_value: str) -> Iterator[dict[str, object]]:
    ...
    for item in transaction_list:
        if item.get("operationAmount", {}).get("currency", {}).get("code", {}) == current_value.upper():
            yield item
...
def card_number_generator(start: int, stop: int) -> Iterator[str]:
    ...
    start_modul = abs(start)
    stop_modul = abs(stop)
    if start_modul > stop_modul:
        start_modul, stop_modul = stop_modul, start_modul

    for num in range(start_modul, stop_modul + 1):
